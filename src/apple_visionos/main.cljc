(ns apple-visionos.main
  "Kotodama WASM entrypoint for the apple_visionos clean-room actor (L4) — Clojure port.
  ...
  facts (`apple_visionos.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "apple_visionos")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"apple_visionos.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "apple_visionos-compat" :tier tier :entities entities} 200])
...
(defn start! [] :apple_visionos-compat/ready)
